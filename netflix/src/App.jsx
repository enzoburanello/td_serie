        ...
        <div className=" h-screen">
          <div className="relative font-quicksand">
            ...
            <div className="relative">
              <img className="w-full h-auto object-cover md:h-screen lg:h-screen" src={data[0].show.image?.original} alt={data[0].show.name} />
              <div className="absolute inset-0 bg-grad z-0"></div>
            </div>


            <div className="absolute inset-0 flex flex-col justify-between z-10 w-full py-5 px-5 text-white lg:px-20 lg:gap-10">
                ...
                <Popover open={open} onOpenChange={setOpen}>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      role="combobox"
                      aria-expanded={open}
                      className=" flex w-9 justify-center bg-lightBlack bg-opacity-50 border-none"
              ...
              <div className="flex flex-col gap-3 w-full lg:w-3/4 md:3/4 lg:pb-36">
                <h1 className=" text-4xl font-bebas uppercase lg:text-7xl">{data[0].show.name}</h1>
                <p className="text-xs lg:text-lg ">{data[0].show.summary?.replace(/<\/?[^>]+(>|$)/g, "")}</p>
                <div className="flex gap-4 text-xs md:text-lg lg:text-lg">
                  <p className="bg-darkGray px-2 py-1 rounded">{data[0].show.genres.join(", ")}</p>
                  <p className="bg-darkGray px-2 py-1 rounded">{data[0].show.premiered}</p>
                  <p className="bg-darkGray px-2 py-1 rounded">{data[0].show.rating?.average}/10</p>
                </div>
              </div>
            </div>
