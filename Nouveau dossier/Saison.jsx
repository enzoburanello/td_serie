    ...
    return (
      <div className="flex flex-col gap-5 ">
        ...
        {filteredEpisodes.length > 0 ? (
          <ul className=" flex flex-col gap-5   text-blanc   ">

            {filteredEpisodes.map((episode) => (
              <li className="grid grid-rows-layout grid-cols-layout gap-2 " key={episode.id}>
                ...
                <p className="lg:col-start-2 lg:text-xl col-span-2 font-quicksand text-xs">{episode.summary?.replace(/<\/?[^>]+(>|$)/g, "") || "Pas de résumé disponible."}</p>
